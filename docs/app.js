const translations = {
  'zh-CN': {
    ...
    heroLead: 'SuperClipboard 把剪贴板历史、备忘录、回收站、主题和托盘快捷操作收进一个轻量窗口里。默认本地保存，也可以在存储设置中连接自己的 PostgreSQL。',
    ...
    storageExternalBody: '用户可以保存多个数据库连接并快速切换。外部模式下正文数据写入外部数据库。',
    ...
    supportWindowsBody: 'SuperClipboard 3.1.0',
  ...
  en: {
    ...
    heroLead: 'SuperClipboard keeps clipboard history, memos, recycle bin recovery, themes, and tray shortcuts in one lightweight window. Data stays local by default, with Storage Settings for your own PostgreSQL database.',
    ...
    featureEditBody: 'Clipboard entries can be edited and copied in their latest form; expanded original content remains available to view and copy, while memos use formatted preview by default.',
    ...
    featureThemeBody: 'Light, dark, system mode, and blue/pink accents are supported. Experimental settings can use matching category colors for selected Tabs while memo color stays independent.',
    ...
    storageLead: 'Data is stored in local SQLite by default. For multi-device plans, use Storage Settings to connect your own PostgreSQL database.',
    ...
    storageExternalBody: 'Users can save multiple database connections and switch between them quickly. In External mode, body content is written to the external database.',
