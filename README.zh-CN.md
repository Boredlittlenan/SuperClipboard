...
- `SuperClipboard_2.15.1_x64-setup.exe`：推荐使用的 Windows 安装器
- `SuperClipboard_2.15.1_x64_en-US.msi`：MSI 安装包
...
- macOS：计划支持。等有 Mac 开发设备后，开始做适配、测试和打包。
...
- 自 v2.15.1 起，检查更新会先显示 Release 更新说明。
...
- 重新设计备份/恢复格式，优先考虑 SQLite 或压缩归档后再开放设置入口。
- 将备忘录图片从正文 data URL 逐步迁移到独立图片存储/缓存。
- 大量剪贴板历史下引入虚拟列表，降低滚动和渲染压力。
- 补充快捷键录制、托盘行为、检查更新和备忘录图片排序的端到端测试。
- 在交互稳定后重新设计插入符跟随和保存窗口位置功能。
- 有 Mac 开发设备后启动 macOS 版本适配。
