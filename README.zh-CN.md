...
- 备忘录图片粘贴：在备忘录编辑区 Ctrl+V 粘贴图片，以 base64 内嵌存储，预览区自动渲染
- 剪贴板内容编辑：在线编辑，自动保留原内容并可折叠展开对比
- 原格式预览开关：以等宽字体完整显示剪贴板内容，不截断
- 自动检查更新开关：可在设置中开启启动时自动检查更新
- 时间显示优化：超过 24 小时的条目显示具体日期时间（如 "6/24 15:30"）
- 紧凑设置面板：各选项悬浮显示详细说明
- 主题三合一按钮：单行三段式切换（浅色 / 深色 / 自动）
- 备忘录独立暖色调视觉风格，所有主题下清晰可辨
- 自定义备忘录颜色：8 种预设色 + HEX 自定义输入，独立于主题
- 回收站：软删除，回收站 Tab 内分「剪贴板」和「备忘录」两个子标签，30 天自动清理
- 回收站倒计时：条目显示剩余天数，黄色徽章提示
- 保存窗口位置：记录上次窗口位置，下次启动时自动恢复（托盘右键设置重置为默认位置）
- 跟随模式：快捷键唤起窗口时自动定位到插入符附近，智能判断显示方向确保窗口完整可见（托盘点击不触发）
